<x-app-layout>
    ...
    <div class="min-h-screen py-12 bg-white/10 dark:bg-gray-900 transition-colors">
        <div class="max-w-lg mx-auto bg-white/80 dark:bg-gray-800/80 border border-[#0b545b]/20 dark:border-gray-700 shadow-lg p-8 rounded-2xl">
            <form action="{{ route('faculties.store') }}" method="POST" class="space-y-6" novalidate autocomplete="off">
                @csrf
                <div>
                    <label for="name" class="block text-sm font-medium text-[#231f20] dark:text-gray-200 mb-1">
                        {{ __('Nombre de la Facultad') }}
                    </label>
                    <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="e.g.: Facultad de Ciencias" class="block w-full px-4 py-2 bg-white/80 dark:bg-gray-700 border border-[#0b545b]/20 dark:border-gray-600 rounded-lg text-[#231f20] dark:text-gray-100 placeholder-[#0b545b]/50 dark:placeholder-[#ffffff]/20 focus:outline-none focus:ring-2 focus:ring-[#31c0d3] focurs:border-[#31c0d3] transition"/>
